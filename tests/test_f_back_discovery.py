...
def test_get_f_back_offset_not_a_frame() -> None:
    ...
    with patch("offline_debug.serializer._py_frame_new", return_value="not a frame"):
        offset = _get_f_back_offset()
...
def test_get_f_back_offset_exception_in_try() -> None:
    ...
    with patch(
        "offline_debug.serializer._py_thread_state_get", side_effect=RuntimeError("thread error")
    ):
        offset = _get_f_back_offset()
        assert offset is None
...
def test_get_f_back_offset_wrong_offset_restoration() -> None:
    ...
    # Let's try this:
    with (
        patch("offline_debug.serializer._py_frame_new", return_value=frame),
        patch("offline_debug.serializer.range", return_value=[ptr_size * 10]),
    ):  # Offset 80
